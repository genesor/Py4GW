...
# This script is intended to be a showcase of every Methods and all the data that can be accessed from Py4GW
# current status, not complete

module_name = "Py4GW DEMO 2"
...
class Feature:
    def __init__(self, name: str, isWindowOpen: bool, description: str, showWindowFunc: Callable[[], Optional[bool]], values: list[Any]):
        self.name = name
        self.isWindowOpen = isWindowOpen
        self.description = description
        self.showWindow = showWindowFunc
        self.values = values
...
def ShowPyImGuiExtraMaplWindow():
    ...
    try:
        ...
        if PyImGui.begin(PyMap_Extra_InfoWindow_state.window_name, PyImGui.WindowFlags.NoResize):
            ...
            map_instance = GLOBAL_CACHE.Map.GetInstance()
...
# mainWindow holds the configuration for the main demo window, including its name and a list of
# Feature, each representing a feature (like PyImGui, PyMap, etc.) with associated state, description, and its display function.
# Each feature display toggling is handled via a ImGui.toggle_button
# The display logic lives in the DrawMainWindow function.
mainWindow = Window("Py4GW Lib DEMO", [
    Feature(
        name="PyImGui",
        isWindowOpen=False,
        description="Provides bindings for creating and managing graphical user interfaces within the game using ImGui. \nIncludes support for text, buttons, tables, sliders, and other GUI elements.",
        showWindowFunc=ShowPyImGuiDemoWindow,
        values=[],
    ),
    Feature(
        name="PyMap",
        isWindowOpen=False,
        description="Manages map-related functions such as handling travel, region data, instance types, and map context. \nIncludes functionality for interacting with server regions, campaigns, and continents.",
        showWindowFunc=ShowPyMapWindow,
        values=[],
    ),
    Feature(
        name="PyAgent",
        isWindowOpen=False,
        description="Handles in-game entities (agents) such as players, NPCs, gadgets, and items. \nProvides methods for manipulating and interacting with agents, including movement, targeting, and context updates.",
        showWindowFunc=ShowPyAgentWindow,
        values=[],
    ),
    Feature(
        name="PyPlayer",
        isWindowOpen=False,
        description="Provides access to the player-specific operations.\nIncludes functionality for interacting with agents, changing targets, issuing chat commands, and other player-related actions such as moving or interacting with the game environment.",
        showWindowFunc=ShowPlayerWindow,
        values=[],
    ),
    Feature(
        name="PyParty",
        isWindowOpen=False,
        description="Manages party composition and party-related actions.\n This includes adding/kicking party members (players, heroes, henchmen), flagging heroes, and responding to party requests.\nAllows access to party details like members, size, and mode (e.g., Hard Mode).",
        showWindowFunc=ShowPartyWindow,
        values=[],
    ),
    Feature(
        name="PyItem",
        isWindowOpen=False,
        description="Provides functions for handling in-game items.\nThis includes retrieving item information (modifiers, rarity, type), context updates, and operations like dyeing or identifying items.",
        showWindowFunc=ShowItemWindow,
        values=[],
    ),
    Feature(
        name="PyInventory",
        isWindowOpen=False,
        description="Manages the player's inventory, including Xunlai storage interactions, item manipulation (pick up, drop, equip, destroy), and salvage operations. \nAlso includes functions for managing gold and moving items between inventory bags.",
        showWindowFunc=ShowInventoryWindow,
        values=[],
    ),
    Feature(
        name="PySkill",
        isWindowOpen=False,
        description="Handles in-game skills and their properties.\nProvides access to skill-related data such as skill effects, costs (energy, health, adrenaline), and professions.\nIncludes methods for interacting with individual skills and loading skill templates.",
        showWindowFunc=ShowSkillWindow,
        values=[],
    ),
    Feature(
        name="PySkillbar",
        isWindowOpen=False,
        description="Manages the player's skillbar, including loading skill templates, using skills in specific slots, and refreshing the skillbar context. \nEach skill in the skillbar can be interacted with or updated.",
        showWindowFunc=ShowSkillbarWindow,
        values=[],
    ),
    Feature(
        name="PyEffects",
        isWindowOpen=False,
        description="Provides functions for handling in-game effects, including conditions, hexes, enchantments, and other status effects. \nIncludes methods for removing, and updating effects on the player.",
        showWindowFunc=ShowEffectsWindow,
        values=[],
    ),
    Feature(
        name="PyMerchant",
        isWindowOpen=False,
        description="Manages interactions with in-game merchants, including buying and selling items, requesting price quotes, and checking transaction status. \nProvides methods to handle trade-related actions and merchant-specific functionality.",
        showWindowFunc=ShowMerchantWindow,
        values=[],
    ),
    Feature(
        name="PyQuest",
        isWindowOpen=False,
        description="Provides functions for handling in-game quests. \nIncludes methods for handling active quests and abandoning them, more info is yet to be shared.",
        showWindowFunc=ShowQuestWindow,
        values=[],
    ),
    Feature(
        name="Py4GW",
        isWindowOpen=False,
        description="Is a collection of miscellaneous functions that are not covered by the other modules.",
        showWindowFunc=ShowPy4GW_Window_main,
        values=[],
    ),
])
...
def DrawMainWindow():
    """
    Draws the main GUI window for the Py4GW demo.

    This function displays a dynamic grid of toggle buttons, one for each feature provided by Py4GW and added as Feature to
    .
    When a feature's button is toggled on, the title and description of that feature are shown in a section below the buttons,
    and the corresponding feature window is displayed.

    The function also handles exception logging using Py4GW's console in case any errors occur while drawing the window.
    """
    global module_name
    global mainWindow
    ...
    try:
        ...
        PyImGui.set_next_window_size(width, height)
        if PyImGui.begin(mainWindow.name, PyImGui.WindowFlags.NoResize):

            # ----- Top Section: Dynamic Tileset of Buttons -----
            PyImGui.text("Select a Feature:")

            # Calculate dynamic grid layout based on number of buttons
            total_buttons = len(mainWindow.features)
            columns, rows = calculate_grid_layout(total_buttons)

            # Create a table with dynamically calculated columns
            if PyImGui.begin_table("MainWindowButtonTable", columns):  # Dynamic number of columns
                for feature_label in mainWindow.features:
                    PyImGui.table_next_column()  # Move to the next column\

                    fName = mainWindow.features[feature_label].name
                    mainWindow.features[feature_label].isWindowOpen = ImGui.toggle_button(fName, mainWindow.features[feature_label].isWindowOpen)

                    if mainWindow.features[feature_label].isWindowOpen:
                        title =mainWindow.features[feature_label].name
                        explanation_text_content = mainWindow.features[feature_label].description
            ...
            for feature_label in mainWindow.features:
                if mainWindow.features[feature_label].isWindowOpen:
                    mainWindow.features[feature_label].showWindow()
...
def CloseAllWindows():
    global mainWindow, ImGui_window_state, ImGui_selectables_window_state, ImGui_input_fields_window_state, ImGui_tables_window_state
    ...
    for feature_label in mainWindow.features:
        mainWindow.features[feature_label].isWindowOpen = False
...
# main function must exists in every script and is the entry point for your script's execution.
def main():
    global module_name
    try:
        if GLOBAL_CACHE.Map.IsMapReady():
...
# Function required to be correctly loaded as a Widget.
def configure():
    pass
